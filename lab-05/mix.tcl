...
# Start mix flows i -> i+1
#
if { $val(run_tcp) == 1 } {
    for {set i 1} {$i < $val(nn)} {incr i} {
	attach-tcp-traffic $i [expr $i+1] $val(packetSize) $val(start0) $val(stop0) $i
	puts "attached $i->[expr $i+1]"
	incr i
	
    }
} else {
    for {set i 1} {$i < $val(nn)} {incr i} {
	attach-cbr-traffic $i [expr $i+1] $val(packetSize) $val(sendingRate) $val(start0) $val(stop0) $i
	incr i 
    }
}
