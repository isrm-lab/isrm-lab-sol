...
# Start downlink flows 2..n -> 1
#
if { $val(run_tcp) == 1 } {
    for {set i 2} {$i < $val(nn)} {incr i} {
	 attach-tcp-traffic $i 1 $val(packetSize) $val(start0) $val(stop0) $i
	puts "attached 0->$i"
    }
} else {
    for {set i 2} {$i < $val(nn)} {incr i} {
	attach-cbr-traffic $i 1 $val(packetSize) $val(sendingRate) $val(start0) $val(stop0) $i
    }
}
