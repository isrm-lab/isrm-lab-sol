...
# Start uplink flows 1 -> 2..n
#
if { $val(run_tcp) == 1 } {
    for {set i 2} {$i < $val(nn)} {incr i} {
	 attach-tcp-traffic 1 $i $val(packetSize) $val(start0) $val(stop0) $i
	puts "attached 0->$i"
    }
} else {
    for {set i 2} {$i < $val(nn)} {incr i} {
	attach-cbr-traffic 1 $i $val(packetSize) $val(sendingRate) $val(start0) $val(stop0) $i
    }
}
