...
if { $val(run_tcp) == 1 } {
    for {set i 1} {$i < $val(nn)} {incr i} {
	 attach-tcp-traffic 0 $i $val(packetSize) $val(start0) $val(stop0) "$i.dn"
	 attach-tcp-traffic $i 0 $val(packetSize) $val(start0) $val(stop0) "$i.up"
	puts "attached 0->$i"
    }
} else {
    for {set i 1} {$i < $val(nn)} {incr i} {
	attach-cbr-traffic 0 $i $val(packetSize) $val(sendingRate) $val(start0) $val(stop0) "$i.dn"
	attach-cbr-traffic $i 0 $val(packetSize) $val(sendingRate) $val(start0) $val(stop0) "$i.up"
    }
}
